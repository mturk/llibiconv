...
PROJECT = iconv-1
WORKDIR = $(BUILD_CPU).$(TARGET)
LFLAGS  = /nologo /SUBSYSTEM:CONSOLE /MACHINE:$(BUILD_CPU) $(EXTRA_LFLAGS)
!ENDIF
OUTPUT  = $(WORKDIR)\$(PROJECT).$(TARGET)

CLOPTS = /c /nologo /wd4244 /wd4267 /wd4090 $(CRT_CFLAGS) -W3 -O2 -Ob2
RFLAGS = /l 0x409 /n /d NDEBUG /d WIN32 /d WINNT
RFLAGS = $(RFLAGS) /d _WIN32_WINNT=$(WINVER) /d WINVER=$(WINVER) $(EXTRA_RFLAGS)
